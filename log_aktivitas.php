...
// Mengambil data log (Jika tabel belum ada, buat penampung aman agar tidak error)
$qLog = mysqli_query($koneksi, "SHOW TABLES LIKE 'tb_log'");
$adaTabelLog = ($qLog && mysqli_num_rows($qLog) > 0);
$dataLog = $adaTabelLog ? mysqli_query($koneksi, "SELECT * FROM tb_log ORDER BY id DESC LIMIT 50") : null;
?>
<!DOCTYPE html>
<html lang="id">
<head>
  <meta charset="UTF-8">
  <title>Log Aktivitas - Coffe Sedayu</title>
...
<body>
  <sidebar>
    ...
    <div class="sidebar-menu">
      ...
      <a href="log_aktivitas.php" class="menu-item active"><i class="fa-solid fa-clock-rotate-left"></i> <span>Log Aktivitas</span></a>
  ...
  <div class="main-content">
    <div class="top-bar">
      <div><h1>Log Aktivitas Sistem</h1><p>Catatan aktivitas pengguna dan sistem parkir.</p></div>
    </div>
    <div class="glass-card">
      <table>
        <thead>
          <tr><th>No</th><th>Waktu</th><th>Aktivitas / Keterangan</th></tr>
        </thead>
        <tbody>
          <?php if ($adaTabelLog && $dataLog && mysqli_num_rows($dataLog) > 0): ?>
            <?php $no = 1; while ($l = mysqli_fetch_assoc($dataLog)): ?>
              <tr>
                <td><?= $no++; ?></td>
            ...
            <?php endwhile; ?>
          <?php else: ?>
            <tr><td colspan="3" style="text-align:center; color:var(--text-sub);">Belum ada riwayat log aktivitas tercatat di database.</td></tr>
          <?php endif; ?>
        </tbody>
      </table>
    </div>
